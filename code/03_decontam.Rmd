---
title: "Screening of Contaminating features"
author: Yanxian Li
date: "`r Sys.time()`"
output: 
  html_document: 
...
# Introduction
The screening will be based on two typical characteristics of Contaminating sequences as outlined in the paper by [Davis et al. (2018)](https://microbiomejournal.biomedcentral.com/articles/10.1186/s40168-018-0605-2): they are likely to have frequencies that inversely correlate with sample DNA concentration and are likely to have higher prevalence in control samples than in true samples. The authors developed an R package, [*decontam*](https://github.com/benjjneb/decontam), for removing Contaminating sequences in the marker-gene and metagenomics data. The package, however, does not make use of positive controls for the identification of Contaminating sequences. As removing of features may heavily affect downstream analyses, we'll do it by manual screening based on the aforementioned principles.
...
pdf(here("data", "qiime2", "decontam", "feature_prevalence.pdf"), width = 16, height = 10) 
...
tab_merged %>% 
  ...
  mutate(cq_mean = map_dbl(data, ~mean(.x$qPCRCqValue)), 
         cq_mean = round(cq_mean, 1)) %>%
  unnest() %>%
...
ggsave(here("result", "figures", "Figure S5.tiff"), width = 8, height = 5, 
...
pdf(here("data", "qiime2", "decontam", "correlation.pdf"), width = 16, height = 12) 
...
# List of Contaminating sequences 
Based on the evaluation of the taxa bar plots, correlation plots and our prior knowledge of common Contaminating taxa found in our lab, the following features are considered as Contaminating sequences in the samples.
```{r contam_sample}
...
# Export Contaminating sequence IDs
Now we export the featureIDs of Contaminating sequences for feature table filtering.
```{r export_contam}
# Contaminating features for samples
write.table(contam_sample, 
            file = here("data", "qiime2", "decontam", "contaminants-sample.tsv"), 
            quote = FALSE, 
            sep = '\t',
            row.names = FALSE)

# Contaminating features for the mock
write.table(contam_mock, 
            file = here("data", "qiime2", "decontam", "contaminants-mock.tsv"), 
            quote = FALSE, 
            sep = '\t',
            row.names = FALSE)
```
