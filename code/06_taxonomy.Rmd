...
Make taxa barplot
```{r}
# Reorder factor "SampleType" so that digesta and mucosa sample from the same fish align vertically
metadata <- rownames_to_column(metadata, "SampleID") %>%
  mutate(SampleType = factor(SampleType, levels = c("REF-DID", "IM-DID", "REF-DIM", "IM-DIM"))) %>%
  column_to_rownames("SampleID")

# Plot taxa
p_phy <- make_taxa_barplot(table = tab_phy, 
                           metadata = metadata, 
                           group_by = SampleType, 
                           ntaxa = 10,
                           nrow = 2,
...
```{r}
p_gen <- make_taxa_barplot(table = tab_gen, 
                           metadata = metadata, 
                           group_by = SampleType, 
                           ntaxa = 10,
                           nrow = 2,
...
plot_grid(p_phy, p_gen, labels = c("A", "B"), ncol = 1, align = 'v', axis = "lr") 

# Export the plot
ggsave(here("result", "figures", "Figure 1.tiff"), width = 10, height = 10, 
       units = "in", dpi = 300, compression = "lzw")
```
...
prev_rd <- subset_samples(phyloseq_s, SampleType == "REF-DID") %>% prevalence() 
prev_rm <- subset_samples(phyloseq_s, SampleType == "REF-DIM") %>% prevalence() 
prev_id <- subset_samples(phyloseq_s, SampleType == "IM-DID") %>% prevalence() 
prev_im <- subset_samples(phyloseq_s, SampleType == "IM-DIM") %>% prevalence() 
```
...
core_taxa_tab %>%
  gt() %>%
  tab_header(title = "Table S2. The prevalence of core taxa in different sample types.") %>%
