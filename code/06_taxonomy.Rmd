...
mock_merged <- bind_cols(mock_exp, mock_obs) %>%
  ...
  mutate_if(is.numeric, ~(100 * .x)) # use percentage as unit for relative abundance
```
...
p_mock <- ggplot(mock_merged, aes(type, abundance)) + 
  ...
  annotate("text", x = c(1, 2.5), y = c(105, 105), label = c("Expected", "Observed"), size = 5) +
  scale_y_continuous(limits = c(0, 105), breaks = 0:10*10, expand = expand_scale(mult = c(0, 0.05))) +
  labs(x = "Sample ID", y = "Relative abundance (%)", fill = "Taxa (expected/observed)") +
...
p_neg <- slice(tab_neg, 1:10) %>% # select the top 10 most abundance taxa for plotting
  ...
  annotate("segment", x = c(0.6, 4.6), xend = c(4.4, 6.4), y = c(102, 102), yend = c(102,102)) +
...
ggsave(here("result", "figures", "Figure 1.tiff"), width = 8, height = 10, 
...
p_phy_ind <- make_taxa_barplot(count_table = tab_phy, metadata = metadata, ntaxa = 10,
                               group = SampleType,  plot_mean = F) 

# Plot taxa using group means
p_phy_mean <- make_taxa_barplot(count_table = tab_phy, metadata = metadata, ntaxa = 10,
                                group = SampleType,  plot_mean = T) + 
                                labs(x = "", y = "")
```
...
```{r}
# Plot taxa on individual basis
p_gen_ind <- make_taxa_barplot(count_table = tab_gen, metadata = metadata, ntaxa = 10, 
                               group = SampleType,  plot_mean = F) 
# Plot taxa using group means
p_gen_mean <- make_taxa_barplot(count_table = tab_gen, metadata = metadata, ntaxa = 10,
                                group = SampleType,  plot_mean = T) +
                                labs(x = "", y = "")
```
...
# Export the plot
ggsave(here("result", "figures", "Figure 2.tiff"), width = 16, height = 10, 
       units = "in", dpi = 300, compression = "lzw")
```
