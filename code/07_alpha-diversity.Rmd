...
phyloseq_s <- tax_glom(phyloseq, "Species")

# Compute Observed features, Peilou's Evenness and Shannon's index
adiv_npd <- alpha(phyloseq_s, index = c("observed", "evenness_pielou", "diversity_shannon")) %>%
  rownames_to_column("SampleID")

# Compute Faith's Phylogenetic Diversity
adiv_pd <- pd(samp = t(otu_table(phyloseq_s)), tree = phy_tree(phyloseq), include.root = F) %>%
  select(PD) %>%
  rownames_to_column("SampleID")
...
adiv <- inner_join(adiv_npd, adiv_pd, by = "SampleID") %>%
  ...
  mutate(FishID = as.character(FishID), NetPen = as.character(NetPen), 
         adiv = factor(adiv, levels = c("observed species", "Pielou's evenness", "Shannon's index", "Faith's PD")))
```
...
Three of the fitted models are singular. Let's look at the fitted models and find out the singular fits.
```{r find-singularFit}
lme
```

From the model outputs, we can see that the standard deviation of random effect `NetPen` for observed species, Pielou's evenness and Faith's PD is zero, which indicates boundary fits. We'll update these models by removing the random effect `NetPen` from the models.
...
The homogeneity of variance for the observed species and Faith's PD is not very good, but we can live with that.
...
anno <- bind_rows(anno1, anno2) %>%
  ...
  arrange(match(adiv, c("observed species", "Pielou's evenness", "Shannon's index", "Faith's PD")),
  ...
  mutate(p.value = ifelse(p.value < 0.001, "***", p.value),
         ...
         y = c(180, 195, 210, 225, 46, 50, 54, 58),
