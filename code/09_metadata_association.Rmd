...
hmp <- maaslin2_heatmap(output_results = here("data", "maaslin2", "all_results.tsv"),
                        first_n = length(unique(sig_result$feature)),
                        cell_value = "qval",
                        data_label = "data",
...
### Boxplot
#### Diet
Make boxplot for Diet effect, color points by SampleOrigin.
```{r boxplot_diet_prepare, fig.width=10, fig.height=26}
...
ggplot(sigAss_diet, aes(x = Diet, y = abundance)) +
  ...
  geom_text(aes(x = Inf, y = Inf, label = ann_boxplot), size = 3, hjust = 1.05, vjust = 1) +
...
#### Sample origin
Make boxplot for SampleOrigin effect, color points by Diet.
```{r boxplot_Sample_origin, fig.width=10, fig.height=32}
# Add number of non-zero observation to text annotation
...
#### Histology
Make boxplot for hisstological scores, color points by SampleOrigin.
```{r boxplot_histology_lpc, fig.width=9, fig.height=3}
# Add number of non-zero observation to text annotation
...
### Scatter plot
#### DISI
Make scatter plot for DISI, color points by SampleOrigin.
```{r scatter_plot_disi, fig.height=3, fig.width=9}
filter(tab_sigAss, metadata == "DISI" ) %>%
  ggplot(aes(x = DISI, y = abundance)) +
    geom_point(aes(colour = Sample_origin), size = 2) +
    geom_text(aes(x = Inf, y = Inf, label = ann_scatter), size = 3, hjust = 1.05, vjust = 1) +
    facet_wrap(~feature, ncol = 2, scales = "free_y") +
    scale_y_continuous(limits = c(0, NA), expand = expand_scale(mult = c(0, 0.5)), 
                       labels = scales::percent_format(accuracy = 0.01)) +   
    labs(y = "Relative abundance") +   
...
filter(tab_sigAss, metadata == "qPCR_barrier_function" ) %>%
  ggplot(aes(x = qPCR_barrier_function, y = abundance)) +
    ...
    scale_y_continuous(limits = c(0, NA), expand = expand_scale(mult = c(0, 0.5)), 
                       labels = scales::percent_format(accuracy = 0.01)) +   
    labs(x = "qPCR barrier function (PC1)", y = "Relative abundance") +  
    theme_cowplot() +
    scale_fill_brewer(palette = "Dark2")
```

## Highlight MaAsLin2 outputs
Highlights of diet effects
```{r highlights_diet}
p_diet <- sigAss_diet %>% 
  filter(feature %in% c("Bacillus", "Corynebacterium 1", "Enterococcus", "Oceanobacillus")) %>%
...
Highlights of sample origin effects
```{r highlights_origin}
p_origin <- sigAss_origin %>% 
  filter(feature %in% c("Photobacterium", "Mycoplasma", "Brevinema andersonii", "f__Spirochaetaceae")) %>%
...
# Assemble ggplots
ind <- plot_grid(p_diet, p_origin, align = "v", ncol = 1, labels = c("B", "C"))
