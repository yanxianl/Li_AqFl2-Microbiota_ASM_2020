...
load("aqfl2_physeq.RData")
```

## Filter Extreemly Low Abundance ASVs
Taxa that were not seen with more than 3 counts in at least 20% of samples are filtered. Subsequently, those with a coefficient of variation ≤ 3 are filtered. Finally we add a pseudocount of 1 to the remaining ASVs to avoid calculating log-ratios involving zeros. Alternatively other replacement methods may be used instead if desired; the subsequent taxa weighting procedure we will describe complements a variety of zero replacement methods. 

```{r, message=FALSE, warning=FALSE}
#physeq <- subset_samples(physeq, Diet == "REF") 
#physeq <- filter_taxa(physeq, function(x) sum(x > 2) > (0.2*length(x)), TRUE)
physeq <- filter_taxa(physeq, function(x) sd(x)/mean(x) > 2.0, TRUE)
physeq <- transform_sample_counts(physeq, function(x) x+1)
```

With these two commands we have removed the filtered taxa from the ASV table, pruned the phylogenetic tree, and subset the taxa table. Here is the result of those filtering steps
```{r, echo=FALSE}
physeq
```
...
4. Convert ASV table to relative abundance (using `philr::miniclo()`) and 'shift' dataset using the weightings [@egozcue2016] using the function `philr::shiftp()`.
...
```{r}
gp.philr <- philr(table_philr, tree, 
...
gp.pcoa <- ordinate(physeq, 'PCoA', distance=gp.dist)
plot_ordination(physeq, gp.pcoa, 
                color='Diet', 
                shape = 'SampleOrignin',
                axes = 1:2) + geom_point(size=4)
```
...
```{r, message=FALSE, warning=FALSE}
sample_data(physeq)$human <- factor(get_variable(physeq, "SampleType") %in% c("Feces", "Mock", "Skin", "Tongue"))
```
...
glmmod <- glmnet(gp.philr, sample_data(physeq)$human, alpha=1, family="binomial")
```
...
```{r}
gp.philr.long <- convert_to_long(gp.philr, get_variable(physeq, 'human')) %>%
